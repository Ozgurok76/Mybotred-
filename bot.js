...
client.elevation = message => {
  ...
  if (message.author.id === Ozgur.sahip) permlvl = 4;
  return permlvl;
};
...
"oynuyor":"RED-BOT-X: Sizi Koruyor",
}
client.ekoayarlar = {
...
if(Date.now() < maxTime) {
  const REDBOTXHerDaim = new Discord.MessageEmbed()
  .setColor(0x36393F)
  .setDescription(` <@${message.author.id}> , **Bu sunucuda spam yapmak yasak!**`)
 // .setFooter(`Bu mesaj otomatik olarak silinecektir.`)
 if (message.member.hasPermission("BAN_MEMBERS")) return ;
 message.channel.send(REDBOTXHerDaim).then(msg => msg.delete({timeout: 1500}));
...
client.on('message', async message => {
  const ms = require('ms');
  const prefix = await require('quick.db').fetch(`prefix_${message.guild.id}`) || REDBOTX.prefix
...
client.on('guildCreate', guild => {
    ...
    let REDBOTX = guild.channels.filter(c => c.type === "text").random()
    REDBOTX1.send("**RED-BOT-X Botunu Sunucuya Eklediğiniz İçin Teşekkürler!** \n !yardım Yazarak Yardım Menüsüne Erişebilirsiniz!");

});
client.on("guildCreate", async guild => {
  const REDBOTX2 = [
    "Botumuzu Sunucuna Eklediğin İçin Teşekkürler!",
    "Bu Bot Ozgur.ok  Tarafından Geliştirilmiştir!",
    '!yardım Yazarak Yardım Menüsüne Ulaşabilirsin'
  ];
  guild.owner.send(REDBOTX2);
